...
**Step 1 — Read the POSCAR**

```
1.000000
  0.00000  2.12855  2.12855
  2.12855  0.00000  2.12855
  2.12855  2.12855  0.00000
Co  O
 1   1
Direct
...
**Step 2 — Derive `lattice_param`**

The lattice vectors are FCC-type: each component = a/2 = 2.12855 Å, so:

```
lattice_param = 2 × 2.12855 = 4.2571 Å
```

**Step 3 — Derive `cell`**

Divide each lattice vector by `lattice_param`:

```
(0, 2.12855, 2.12855) / 4.2571 = (0.0, 0.5, 0.5)
(2.12855, 0, 2.12855) / 4.2571 = (0.5, 0.0, 0.5)
(2.12855, 2.12855, 0) / 4.2571 = (0.5, 0.5, 0.0)
```

**Step 4 — Read `atoms` positions**

Direct (fractional) coordinates in POSCAR map directly to `atoms` positions.

**Step 5 — Set `atoms` magmom**

POSCAR contains no magnetic moment information — set manually based on physics:
- Co is magnetic: give a small non-zero initial moment, e.g., `[0, 0, 0.5]`
- O is non-magnetic: use `[0, 0, 1e-6]` (avoid exact `0` to prevent ASE errors)

**Step 6 — Read k-path from VASPKIT `KPATH.in`**

```
K-Path Generated by VASPKIT.
   20          ← num_kpts per segment
Line-Mode
Reciprocal
  G → X → U | K → G → L → W → X
```

The path string is read from the high-symmetry point labels in order: `"G X U K G L W X"`.

**Result:**

```json
"structure_info": {
    "lattice_param": 4.2571,
    "cell": [
        [0.0, 0.5, 0.5],
        [0.5, 0.0, 0.5],
        [0.5, 0.5, 0.0]
    ...
    "atoms": [
        ["Co", [0, 0, 0],         [0, 0, 0.5]],
        ["O",  [0.5, 0.5, 0.5],   [0, 0, 1e-6]]
    ],
    "kgrid_hse": [10, 10, 10],
    "kgrid_pbe": [10, 10, 10],
    "num_kpts": 20,
    "kpath": "G X U K G L W X",
    "custom_kpoints": null,
...
| POSCAR | input.json |
|--------|------------|
| Scale factor × lattice vector length | `lattice_param` |
| Lattice vectors / `lattice_param` | `cell` |
| Direct fractional coordinates | `atoms` position |
| Not present — set manually | `atoms` magmom |
| KPATH.in line 2 | `num_kpts` |
| KPATH.in high-symmetry labels | `kpath` |
